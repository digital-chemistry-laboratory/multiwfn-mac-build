...
!write(*,*) "5 Extract k-point wavefunction"
read(*,*) isel
...
character,allocatable :: SPlabel(:)*10 !SP labels
...
!Plotting parameters
integer :: ivertline=1,iedgeline=1,ifermiline=0,thkcurve=4,textsize=50
real*8 :: BSxyratio=0.618D0
real*8 :: linex(2),liney(2)
integer :: icurveclr_occ=5,icurveclr_vir=15,icurveclr_occB=11,icurveclr_virB=1
...
read(10,*) c80tmp,nSP
allocate(SPkp(nSP),SPpath(nSP),SPlabel(nSP),SPxyz(3,nSP),SPXpos(nSP))
do iSP=1,nSP
...
if (nvir==0) then
    ...
    write(*,*) "Press ENTER button to continue"
    read(*,*)
    write(*,"(a,f12.6,' eV')") " Valence band top (HOCO):      ",E_HOCO
    E_LUCO=E_HOCO
else
    write(*,"(/,a,f12.6,' eV')") " Valence band top (HOCO):      ",E_HOCO
    write(*,"(a,f12.6,' eV')") " Conduction band bottom (LUCO):",E_LUCO
    E_gap=E_LUCO-E_HOCO
    write(*,"(a,f12.6,' eV')") " Band gap:                     ",max(E_gap,0D0)
    if (E_LUCO<=E_HOCO) then
...
do while(.true.)
    ...
    if (ifermiline==0) write(*,*) "5 Toggle showing horizontal lines highlighting Fermi level, current: No"
    if (ifermiline==1) write(*,*) "5 Toggle showing horizontal lines highlighting Fermi level, current: Yes"
    write(*,*) "6 Set colors"
    ...
    if (isel==-3) then
        ...
        if (ifermiline==0) then
            ifermiline=1
            write(*,*) "Input energy of Fermi level, e.g. 0"
            read(*,*) E_fermi
        else
            ifermiline=0
        end if
    ...
    do iSP=1,nSP
        c80tmp=SPlabel(iSP)
        if (SPlabel(iSP)=="GAMMA") c80tmp="\Gamma"
        if (iSP>1) then
            if (SPkp(iSP)==SPkp(iSP-1)) cycle !This label has already been plotted together last time
        end if
        if (iSP<nSP) then
            if (SPkp(iSP)==SPkp(iSP+1)) c80tmp=trim(SPlabel(iSP))//'|'//trim(SPlabel(iSP+1))
        end if
        CALL TEXMOD("ON")
    ...
    if (ifermiline==1) then !Highlight Fermi level
        CALL DASH
        call setcolor(ihorilineclr)
        CALL LINWID(thkhoriline)
        linex(1)=0
...
!!---------- Obtain LUCO and HOCO energies and show band gap based on the output file containing energy levels of every k-point
subroutine CP2K_bandgap
use defvar
use util
implicit real*8 (a-h,o-z)
